...
client_credentials_manager = SpotifyClientCredentials(
    client_id=api_keys.CLIENT_ID, client_secret=api_keys.CLIENT_SECRET
)
...
def write_data_to_csv(artists_features, file_name, keys=features_to_include):
    ...
    data = {
        artist_name: {k: v for k, v in features.items() if k not in exclude}
        for artist_name, features in artists_features.items()
    }

    data = [
        {"artist_name": artist_name, **features}
